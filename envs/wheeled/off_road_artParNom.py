...
class off_road_art(ChronoBaseEnv):
    ...
    def __init__(self, terrain_stage=0, additional_render_mode='None'):
        try:
            ...
            self.map_offset_x  = 0
            self.map_offset_y  = 0
            self.terrain_file = ["stage_1.bmp", "stage_2.bmp", "stage_3.bmp", "stage_4.bmp", "stage_5.bmp"]
            ...
            self.swae.load_state_dict(torch.load(os.path.dirname(
                os.path.realpath(__file__)) + "/../utils/SWAE_64.pth"))
            ...
            self.min_vectorMCL = torch.tensor(np.load(os.path.dirname(
                os.path.realpath(__file__)) + "/../utils/min_vectorMCL.npy")).to(self.device)
            self.max_vectorMCL = torch.tensor(np.load(os.path.dirname(
                os.path.realpath(__file__)) + "/../utils/max_vectorMCL.npy")).to(self.device)
    ...
    def get_observation(self):
        ...
        try:
            ...
            # # Get vehicle speed and normalize it to the range [-1, 1]
            # vehicle_speed = self.m_chassis_body.GetPos_dt().Length()
    ...
    def initialize_vw_pos(self, seed):
        ...
        if not self.m_isFlat:
            ...
            x, y = self.m_terrain_length * 0.5 * \
                np.cos(theta), self.m_terrain_width * 0.5 * np.sin(theta)
            z = self.m_terrain.GetHeight(chrono.ChVectorD(x, y, 0)) + 2
            ang = np.pi + theta
            self.m_initLoc = chrono.ChVectorD(x, y, z)
            self.m_initRot = chrono.Q_from_AngZ(ang)
            self.m_vehicle.SetInitPosition(
    ...
    def set_goal(self, art_theta, seed):
        ...
        :param seed: Seed for the random number generator
        """
        if self.current_terrain_stage == 2 or self.current_terrain_stage == 3 or self.current_terrain_stage == 4:
            ...
            i = 0
            while (self.m_goal - self.m_initLoc).Length() < 19:
        ...
        elif self.current_terrain_stage == 0 or self.current_terrain_stage == 1:
            # Random angle between -pi/2 and pi/2
            delta_theta = (random.random() - 0.5) * 1.0 * np.pi
            # Goal is always an angle between -pi/2 and pi/2 from the art
            gx, gy = self.m_terrain_length * 0.5 * np.cos(art_theta + np.pi + delta_theta), self.m_terrain_width * 0.5 * np.sin(
                art_theta + np.pi + delta_theta)
            self.m_goal = chrono.ChVectorD(
                gx, gy, self.m_terrain.GetHeight(chrono.ChVectorD(gx, gy, 0)) + 1.1)
            ...
            while (self.m_goal - self.m_initLoc).Length() < 19:
                ...
                gy = random.random() * self.m_terrain_width - self.m_terrain_width / 2
                self.m_goal = chrono.ChVectorD(
                    gx, gy, self.m_terrain.GetHeight(chrono.ChVectorD(gx, gy, 0)) + 1.6)
                if i > 100:
                    print('Failed setting goal randomly, using default')
                    gx = self.m_terrain_length * 0.625 * \
                        np.cos(art_theta + np.pi + delta_theta)
    ...
    def load_terrain(self):
        ...
        terrain_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 
                                "../data/terrain_bitmaps/Manual-CL",
    ...
    def update_terrain_stage(self, stage):
        self.current_terrain_stage = stage
