...
def evaluate_model(env, model, num_trials=20, max_steps=1000, render=False):
    success_count = 0
    traversal_times = []
    episode_roll_means = []
    episode_pitch_means = []
...
if __name__ == '__main__':
    env = off_road_art()
    #Evalue on the one training level
    env.update_terrain_stage(level_index=94)
    
    checkpoint_dir = '../train/logs/vws_ppo_checkpoints_PLRNorm/'
    loaded_model = PPO.load(os.path.join(checkpoint_dir, f"ppo_checkpoint_iter88_level94"), env)
    ...
    for step in range(totalSteps):
        ...
        obs_tensor = th.tensor(obs, dtype=th.float32).to(loaded_model.device)
        print("Pred Value: ", loaded_model.policy.predict_values(obs_tensor).cpu().detach().numpy().item())
        print(f"Step {step + 1}")
        print("Action: ", action)
    ...
    # num_trials = 20
    # success_count, mean_traversal_time, avg_roll_angle, avg_pitch_angle = evaluate_model(env, loaded_model, num_trials=num_trials, render=render)
